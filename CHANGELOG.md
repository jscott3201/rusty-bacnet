...
- Add a five-field arm to `impl_intrinsic_reporting!` for feedback-driven detectors behind an `Event_Detection_Enable` gate, alongside the existing three-field arm. The arms differ in arity and cannot shadow one another, and the seven object types on the three-field form expand unchanged. There is deliberately **no** four-field (feedback, ungated) arm: one existed briefly during this work and became unused once both feedback-driven types moved to the gated form, and leaving it exported would have offered downstream implementors a supported way to wire a three-input detector with detection permanently on — the exact defect the gate was added to fix.
