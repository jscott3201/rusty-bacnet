...
- Scope the MSRV gate to the nine published crates, and take `sysinfo` 0.38 → 0.39. **The MSRV is unchanged at 1.93.** The `msrv` job ran `cargo check --workspace --exclude rusty-bacnet`, which also checked `bacnet-benchmarks` and `bacnet-integration-tests` — neither of which is published, so neither is something a crates.io consumer can depend on. That made an unpublished benchmarking dependency able to dictate the MSRV promised to consumers: `sysinfo` 0.39 declares `rust-version = "1.95"`, so the workspace was held at `sysinfo` 0.38 to keep a gate that had no business covering it green. The job now names the nine published crates explicitly (`bacnet-types`, `bacnet-encoding`, `bacnet-services`, `bacnet-transport`, `bacnet-network`, `bacnet-client`, `bacnet-objects`, `bacnet-server`, `bacnet-cli`); an `--exclude` list would silently absorb any future workspace member, whereas this one cannot drift without being edited. Note the resolver is `"2"`, which is not MSRV-aware, so `sysinfo` 0.39.6 resolves fine under `rust-version = "1.93"` — cargo rejects it at build time (`rustc 1.93.1 is not supported by the following package`), which is why this surfaced as a CI failure rather than a resolution one. Verified on 1.93.1: the nine published crates check clean (exit 0) while the previous command fails on `sysinfo`, and `bacnet-benchmarks` builds on the pinned 1.97.1 toolchain.
