...
- Bring every dependency current. `cargo update` moves 94 in-range packages, including tokio 1.52.3 → 1.53.1, bytes 1.11.1 → 1.12.1, rustls 0.23.40 → 0.23.42, serde 1.0.228 → 1.0.229, serde_json 1.0.149 → 1.0.151, clap 4.6.1 → 4.6.4, libc 0.2.186 → 0.2.189, aws-lc-rs 1.16.3 → 1.17.3, socket2 0.6.3 → 0.6.5, and thiserror 2.0.18 → 2.0.19. Two semver-major bumps follow, neither of which needed a source change: `rustyline` 17 → 18 in `bacnet-cli`, and `sysinfo` 0.38 → 0.39 in the benchmarks. The lock shrinks from 328 packages to 311, largely because the `wit-bindgen`/`wit-component`/`wasm-metadata` build chain leaves — which also clears the single allowed `cargo audit` warning this workspace had been carrying, so `cargo audit` now reports no findings at all. Duplicate-version warnings from `cargo deny` drop from 10 to 3. All ten on `dev` were Windows-family crates — `windows-sys` plus `windows-targets` and eight `windows_*` target shims — and every one is resolved: `rustyline` 18 drops `fd-lock` and the `sysinfo` bump clears the rest, taking locked `windows-sys` from five versions to three and `windows_x86_64_msvc` from three to two. Of the three remaining, `syn` is new and not ours to resolve (`serde_derive`, `clap_derive`, and `thiserror-impl` have moved to `syn` 3 while `tokio-macros`, `tracing-attributes`, `futures-macro`, and `zerocopy-derive` still require `syn` 2, so both majors stay locked until those crates migrate); `getrandom` and `r-efi` are newly reported but their locked version sets are unchanged, so nothing new was pulled in. `cargo deny` reports advisories, bans, licenses, and sources all ok.
