...
use bacnet_benchmarks::helpers::{current_rss_bytes, make_bip_client, make_bip_server};

fn bench_read_property_latency(c: &mut Criterion) {
    let rt = Runtime::new().unwrap();

    let (mut server, mut client, server_mac) = rt.block_on(async {
        let server = make_bip_server().await.unwrap();
        let client = make_bip_client().await.unwrap();
    ...
    c.bench_function("bip_read_property_latency", |b| {
...
fn bench_write_property_latency(c: &mut Criterion) {
    ...
    c.bench_function("bip_write_property_latency", |b| {
        b.to_async(&rt).iter(|| {
...
fn bench_rpm_latency(c: &mut Criterion) {
    ...
    c.bench_function("bip_rpm_10_objects_latency", |b| {
        b.to_async(&rt).iter(|| {
            let s = specs.clone();
            async {
