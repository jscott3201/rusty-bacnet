...
fn bench_sc_read_property_latency(c: &mut Criterion) {
    let rt = Runtime::new().unwrap();

    let certs = generate_test_certs();
    let hub_vmac = [0xF0; 6];
    let server_vmac = [0x01; 6];
    ...
    c.bench_function("sc_read_property_latency", |b| {
    ...
    eprintln!(
        "SC RSS: before={}KB after={}KB delta={}KB",
        rss_before / 1024,
        rss_after / 1024,
...
fn bench_sc_write_property_latency(c: &mut Criterion) {
    let rt = Runtime::new().unwrap();
    ...
    c.bench_function("sc_write_property_latency", |b| {
        b.to_async(&rt).iter(|| {
...
fn bench_sc_rpm_latency(c: &mut Criterion) {
    let rt = Runtime::new().unwrap();

    let certs = generate_test_certs();
    let hub_vmac = [0xF2; 6];
    let server_vmac = [0x05; 6];
    let client_vmac = [0x06; 6];
    ...
    c.bench_function("sc_rpm_10_objects_latency", |b| {
        b.to_async(&rt).iter(|| {
            let s = specs.clone();
            async {
...
criterion_group!(
    benches,
    bench_sc_read_property_latency,
    bench_sc_write_property_latency,
    bench_sc_rpm_latency
);
